...
struct Metric {
  ...
  this( int index, string label, string description ) {
    this.index = index;
    this.label = toUTFz!( TCHAR * )( label );
    this.labelLen = label.length;
    this.description = toUTFz!( TCHAR * )( description );
...
nothrow LRESULT trapKey( HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam ) in {
  assert( message == WM_KEYDOWN );
} body {  
  switch( wParam ) {
    case VK_HOME:
      return SendMessage( hwnd, WM_VSCROLL, SB_TOP, 0 );      
    case VK_END:
      return SendMessage( hwnd, WM_VSCROLL, SB_BOTTOM, 0 );
    case VK_PRIOR:
      return SendMessage( hwnd, WM_VSCROLL, SB_PAGEUP, 0 );
    case VK_NEXT:
      return SendMessage( hwnd, WM_VSCROLL, SB_PAGEDOWN, 0 );
    case VK_UP:
      return SendMessage( hwnd, WM_VSCROLL, SB_LINEUP, 0 );
    case VK_DOWN:
      return SendMessage( hwnd, WM_VSCROLL, SB_LINEDOWN, 0 );
    case VK_LEFT:
      return SendMessage( hwnd, WM_HSCROLL, SB_PAGELEFT, 0 );
    case VK_RIGHT:
      return SendMessage( hwnd, WM_HSCROLL, SB_PAGERIGHT, 0 );
    default:
    ...
    switch( message ) {
      ...
      case WM_PAINT:
        ...
        for( size_t i = paintBegin; i <= paintEnd; ++i ) {
          ...
          TextOut( hdc, x, y, metric.label, metric.labelLen );
          TextOut( hdc,x + ( 22 * cxCaps ), y, metric.description, metric.descriptionLen );
          SetTextAlign( hdc, TA_RIGHT | TA_TOP );
          auto value = GetSystemMetrics( metric.index );
          string valueStr;
          valueStr = value.to!string;
          TextOut( hdc, x + ( 22 * cxCaps ) + ( 40 * cxChar ), y, valueStr.toUTFz!( TCHAR * ), valueStr.length );
      ...
      case WM_LBUTTONDOWN:
        ...
        DrawText( hdc, "WOOOOOOOOOOOOOOOOOOOOOT".toUTFz!( TCHAR * ), -1, &rect, DT_SINGLELINE | DT_CENTER | DT_VCENTER );
        EndPaint( hwnd, &ps );
        return 0;
      case WM_KEYDOWN:
        return trapKey( hwnd, message, wParam, lParam );
      case WM_DESTROY:
        PostQuitMessage( 0 );
        return 0;    
      default:
